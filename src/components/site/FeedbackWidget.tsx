...
export function FeedbackWidget() {
  ...
  return (
    <div data-fb-ui className="font-sans">
            ...
            p.xPct == null || p.yPct == null ? null : (
              <button
                ...
                className={`pointer-events-auto absolute -translate-x-1/2 -translate-y-1/2 flex size-6 items-center justify-center rounded-full text-[11px] font-semibold text-white shadow-lg ring-2 ring-white transition ${
                  p.status === "resolved" ? "bg-olive" : "bg-burgundy"
                }`}
              ...
              >
                ...
                {openPin === p.id && (
                  <span className="pointer-events-none absolute bottom-full left-1/2 mb-2 w-56 -translate-x-1/2 rounded-lg border border-rule bg-white px-3 py-2 text-left text-[12px] font-normal leading-snug text-foreground shadow-xl">
                    {p.note}
                  </span>
                )}
              </button>
      ...
      {commentMode && !draft && hoverRect && (
        <div
          data-fb-ui
          aria-hidden
          className="pointer-events-none fixed z-[9993] rounded-[3px] border-2 border-burgundy bg-burgundy/10 transition-[top,left,width,height] duration-75"
      ...
      {draft && scope === "section" && draft.sectionRect && (
        <div
          data-fb-ui
          aria-hidden
          className="pointer-events-none absolute z-[9993] rounded-[3px] border-2 border-burgundy bg-burgundy/10"
      ...
      {draft && (
        <div
          data-fb-ui
          className="fixed z-[9996] w-[300px] rounded-xl border border-rule bg-white p-3 shadow-2xl"
          style={{ left: Math.max(12, editorLeft), top: Math.max(12, editorTop) }}
        >
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[10px] font-medium uppercase tracking-[0.14em] text-muted">Nuevo comentario</span>
            <button type="button" data-fb-ui onClick={() => setDraft(null)} className="text-muted transition hover:text-foreground" aria-label="Cerrar">
              <X className="size-4" />
            </button>
          </div>
          {draft.sectionSelector && (
            <div className="mb-2 inline-flex rounded-lg border border-rule p-0.5 text-[11px]">
              <button
                type="button"
                data-fb-ui
                onClick={() => setScope("element")}
                className={`rounded-md px-2 py-1 transition ${scope === "element" ? "bg-navy text-white" : "text-muted hover:text-foreground"}`}
              >
                Este elemento
              </button>
              <button
                ...
                className={`rounded-md px-2 py-1 transition ${scope === "section" ? "bg-navy text-white" : "text-muted hover:text-foreground"}`}
              >
                Toda la sección
              </button>
            </div>
          ...
          {scope === "element"
            ? draft.elementShow && (
                <p className="mb-2 line-clamp-2 rounded-md bg-background-warm px-2 py-1 text-[11px] text-muted">{draft.elementShow}</p>
              )
            : (
              <p className="mb-2 rounded-md bg-background-warm px-2 py-1 text-[11px] text-muted">Comentario sobre toda la sección resaltada</p>
            )}
          <textarea
            data-fb-ui
            autoFocus
            ...
            rows={3}
            className="w-full resize-none rounded-lg border border-rule bg-white px-2.5 py-2 text-[13px] text-foreground placeholder:text-muted focus:border-navy focus:outline-none"
          />
      ...
      {commentMode ? (
        <button
          ...
          className="fixed bottom-5 left-5 z-[9994] inline-flex items-center gap-2 rounded-full border border-navy bg-white px-4 py-2.5 text-[13px] font-medium text-navy shadow-xl transition hover:bg-background-warm"
        >
          <X className="size-4" />
